...
/**
 * 종합 시황/공시 제목 수집 (NEWS, 평일 08:05~19:35 30분 간격). 전체 시황 피드 1개를 현재 시각부터 과거 방향으로 연속조회해 tb_stock_news 에 넣는다.
 * <p>
 * "실시간" 은 하루 1회 19:30 판단에 가치가 없으므로 30분 주기면 충분하다. 저장 기준 시각은 기사 작성 시각(data_dt+data_tm, KST) 이며 advisor 는
 * 그 값으로 판단 시각 이전 기사만 고른다(룩어헤드 방어). 이미 저장된 최신 작성 시각보다 오래된 페이지가 나오면 멈춘다(증분).
 */
@Slf4j
@Service
...
public class NewsCollectJob implements CollectJob {
  ...
  public void execute(CollectExecution execution) {
    ...
    List<KisNewsTitleResponse.Row> rows = marketDataPort.fetchNewsTitles(now.toLocalDate(), now.toLocalTime(), null,
        properties.getNews().getMaxPages(), execution.context("NEWS"));
    ...
    for (KisNewsTitleResponse.Row row : rows) {
      ...
      if (item.get().publishedAt().isBefore(since)) {
        skippedOld++;
        continue;
      }
      items.add(item.get());
    ...
    log.info("뉴스 제목 수집: fetched={} candidates={} inserted={} skippedOld={} since={}", rows.size(), items.size(), inserted, skippedOld, since);
  }
