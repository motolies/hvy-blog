 ...
 * INDEX → PRICE → VALUATION → INVESTOR → MARKET_INVESTOR → ETF_NAV → STATS(kis.stats.enabled) → CA_HINT → VALIDATE → DERIVED.
...
public class StockDailyPipelineJob implements CollectJob {
  ...
  private final StockMarketInvestorCollectService marketInvestorService;
  ...
  public void execute(CollectExecution execution) {
    ...
    steps.run("MARKET_INVESTOR", () -> marketInvestorService.collectRecent(execution));
